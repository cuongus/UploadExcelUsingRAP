...
*The type definition resembling the structure of the rows in the worksheet selection.
    ...
    lt_att_create = VALUE #( (  %cid_ref  = keys[ 1 ]-%cid_ref
                                   ...
                                   %target   = VALUE #( FOR ls_data IN lt_rows FROM 2 ( %cid       = |{ ls_data-ebeln }{ ls_data-ebelp }|
                                                                                      %is_draft   = keys[ 1 ]-%is_draft
                                                                                      end_user    = sy-uname
...
*    IF lv_valid <> abap_true.
*
*        MODIFY ENTITIES OF zi_ses_parent
*
*
*      MODIFY zses_file_table FROM @( VALUE #( end_user = sy-uname ) ).
*    ENDIF.
